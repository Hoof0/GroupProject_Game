...
int main() {
    ...
    Story* interrogateHarrisonBottle = new Story();
    Story* collectWhiskeyBottle = new Story();
    Story* interrogateAlexCoffee = new Story();
    ...
    interrogatePassengers->setDescription("Harrison, Alex, Clara, or check security footage.");
    //Collect evidence 3, 10
    interrogatePassengers->setChoiceses("Harrison", interrogateHarrisonNote, 0);
    interrogateHarrisonNote->setDescription("When you were wondering who you should go to first, Harrison approached you with a note in his hand.");
    ...
    collectThreatNote->setChoiceses("Follow Harrison back to his seat", interrogateHarrisonBottle, 0);
    interrogateHarrisonBottle->setDescription("When you followed Harrison back to his seat to check his belongings, you caught Harrison and Mika talking about something. You eavesdropped on them, and what you heard piqued your attention. After that, you quickly inspected Harrison's whiskey bottle.");
    interrogateHarrisonBottle->setChoiceses("Inspect the bottle", collectWhiskeyBottle, 0);
    ...
    collectWhiskeyBottle->setChoiceses("Continue interrogating.", interrogatePassengers, 0);
    interrogatePassengers->setChoiceses("Alex", interrogateAlexCoffee, 0);
    interrogateAlexCoffee->setDescription("When you approached Alex's seat, you heard Alex telling Clara that his coffee was drugged. When Clara left, you asked Alex to check his belongings and got his permission.");
    interrogateAlexCoffee->setChoiceses("Inspect Alex's cup of coffee.", collectAlexCoffee, 0);
    ...
    collectAlexPen->setChoiceses("Leave and go interrogate other passengers.", eavesdropAlex, 0);
    eavesdropAlex->setDescription("When you had gathered your evidence, you bid Alex a temporary goodbye. However, right when you turned away, Alex's voice rang from behind you. At first, you thought he was calling you, but then you realized his voice was very small, barely above a whisper, and if yours ears were not fooling you, it contained the words 'payback', 'kill'. You halted your footsteps and pretended to inspect the cabin, while eavesdropping on Alex.");
    ...
    tornPhoto->setDescription("One half shows Victor with a young woman (Ami Sato) at a NovaTech gala; the other half is hidden in Mika's locket, which you have noticed when she served you water.");
